...
export default class ChimeSdkWrapper {
  ...
  async createRoom({ role, name, title, playbackURL, region }) {
    if (!name || !title || !role) {
      logger.error(`role=${role} name=${name} title=${title} must exist`);
      return undefined;
    }
  ...
  async initializeMeetingSession(configuration) {
    const awsLogger = new ConsoleLogger('SDK', LogLevel.ERROR);
        ...
        this.audioVideo.realtimeSubscribeToVolumeIndicator(
          ...
          async (attendeeId, volume, muted, signalStrength) => {
            ...
            if (baseAttendeeId !== attendeeId) {
              ...
              // or any other MediaStream object),
              // the content attendee (attendee-id#content) joins the session and
              // shares content as if a regular attendee shares a video.
              //
              // For example, your attendee ID is "my-id".
              // When you call meetingSession.audioVideo.startContentShare,
  ...
  async joinRoom(element) {
    ...
    try {
      ...
      logger.debug('failed to bind audio element: ', error);
    }
  }
...
export const chime = new ChimeSdkWrapper();
